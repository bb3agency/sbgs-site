...
export function StoreSpotlight() {
  return (
    <section className="relative overflow-hidden" style={{ backgroundColor: "#7F1416" }}>
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute -right-40 -top-20 size-[500px] rounded-full bg-brand-gold opacity-[0.06] blur-3xl"
      />
      ...
      <div className="relative mx-auto w-full max-w-[1440px] px-4 py-16 sm:px-6 sm:py-20 lg:px-8 lg:py-24">
        ...
        <div className="mb-10 text-center lg:mb-14">
          <div className="mb-4 flex items-center justify-center gap-3">
            ...
            <span className="text-[10px] font-bold uppercase tracking-[0.25em] text-brand-gold/60 font-['Montserrat']">
              Our Stores
            </span>
            <div className="h-px w-8 bg-brand-gold/20" />
          </div>
          <h2 className="font-serif text-3xl font-normal text-white sm:text-4xl lg:text-5xl">
            That&apos;s Us in the{" "}
            <em className="italic text-brand-gold">Spotlight</em>
          </h2>
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4 lg:gap-4">
          {SPOTLIGHT_IMAGES.map((img, idx) => (
            <div
              key={idx}
              className={`group relative aspect-[4/3] overflow-hidden bg-card/10 ring-1 ring-white/[0.08] transition-all duration-500 hover:ring-brand-gold/20 hover:shadow-xl ${img.span}`}
            >
              <Image
                src={img.src}
                alt={img.alt}
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t from-brand-maroon/20 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
            </div>
          ))}
        </div>
        ...
        <div className="mt-12 flex flex-col items-center gap-4 text-center">
          ...
          <Link
            href="/stores"
            className="inline-flex items-center gap-2 border border-white/20 px-6 py-2.5 text-sm font-medium uppercase tracking-wider text-white/70 transition-all duration-300 hover:border-brand-gold/40 hover:text-brand-gold font-['Montserrat']"
          >
