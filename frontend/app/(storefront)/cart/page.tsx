...
export default function CartPage() {
  return (
    <div className="flex flex-col bg-background min-h-screen pb-16">
      {/* ── Page Header ──────────────────────────────────────────────────── */}
      <section className="relative overflow-hidden bg-gradient-to-br from-brand-maroon via-brand-maroon to-brand-maroon-dark py-10 md:py-16">
        <div className="absolute -top-16 right-16 size-56 rounded-full bg-brand-maroon opacity-10 blur-3xl" aria-hidden />
        <div className="absolute -bottom-12 -left-12 size-48 rounded-full bg-secondary opacity-15 blur-3xl" aria-hidden />
        ...
        <div className="relative mx-auto flex w-full max-w-[1440px] flex-col items-center justify-center px-4 text-center lg:px-8">
          ...
          <nav className="flex items-center gap-1.5 text-xs font-semibold text-white/60 sm:gap-2 sm:text-sm" aria-label="Breadcrumb">
            ...
            <Link href="/products" className="transition-colors hover:text-brand-gold">Shop</Link>
            <ChevronRight className="size-3" />
